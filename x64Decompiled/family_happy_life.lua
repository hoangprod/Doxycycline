...
local TracingBtnChecked = function(checkBtn, index)
  ...
  local info = X2Achievement:GetTodayAssignmentInfo(TADT_FAMILY, index)
...
local function TodayAssignmentDataSetFunc(subItem, index, setValue)
  ...
  if setValue then
    subItem:SetTodayAssignment(A_SUBJECT_STYLE.TODAY_ASSIGNMENT_LIST, TADT_FAMILY, index, CheckFunc, TracingBtnEntered)
  else
...
local function CreateTodayAssignmentListWnd(id, parent)
  ...
  title:SetText(GetCommonText("family_happy_life_notify"))
  ApplyTextColor(title, FONT_COLOR.BROWN)
  function wnd:SetListSelectFuc(func)
    subjectList:SetListSelectFuc(func)
  end
  function wnd:Init()
    local cnt = X2Achievement:GetTodayAssignmentCount(TADT_FAMILY)
    local info = {}
    for index = 1, cnt do
      info[index] = index
    end
  ...
  function wnd:SetInfo(index, aList, prevPage)
    ...
    elseif info.status == A_TODAY_STATUS.LOCKED then
      ...
      if info.satisfy then
        CallUnlockTodayQuestDialog(GetUIText(COMMON_TEXT, "family_happy_life"), TADT_FAMILY, info)
      end
      return
    elseif info.status == A_TODAY_STATUS.READY then
      ...
      X2Achievement:HandleClickTodayAssignment(TADT_FAMILY, info.realStep)
      return
    end
    wnd.viewInfo = {
      index = index,
    ...
    subjectWnd:SetTodayAssignment(A_SUBJECT_STYLE.TODAY_ASSIGNMENT_DESC, TADT_FAMILY, index, CheckFunc, TracingBtnEntered)
    ...
    local function RefreshMoveBtn()
      ...
      for i = cIndex - 1, 1, -1 do
        local sInfo = X2Achievement:GetTodayAssignmentInfo(TADT_FAMILY, aList[i])
...
function ShowFamilyHappyLifeTab(parent)
  local assignDesc = CreateTodayAssignDescWnd("assignDesc", parent, W_TODAY_ASSIGN_DESC.TYPE.FAMILY)
  ...
  function parent:ToggleWithQuest(qType)
    ...
    for index = 1, #info do
      local tInfo = X2Achievement:GetTodayAssignmentInfo(TADT_FAMILY, info[index])
