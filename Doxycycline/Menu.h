...
struct Consolelogs
{
	...
	Consolelogs()
	{
		...
		AddLog("Welcome to admin console log!");
	}
	~Consolelogs()
	{
		ClearLog();
	...
	{
		Consolelogs* console = (Consolelogs*)data->UserData;
...
struct PacketEditor
{
	...
	{
		std::ofstream Stream;
		Stream.open("Packet.bin", std::ios::binary | std::ios::out);
	...
	{
		std::ofstream Stream;
		Stream.open("Packet.bin", std::ios_base::app);
		Stream.write((char*)DataBuffer, Size);
		Stream << "ZZZZZZZZZZZZZZZZZZZZZZZZZZ";
		Stream.close();
	}

	void Edit(std::vector<char*> pVector, BYTE Element)
	{
